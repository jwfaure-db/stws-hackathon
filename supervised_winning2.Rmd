...
```{r}
for(z in unique(df$AFL_ZONE)){
  X_total <- df %>% dplyr::filter(AFL_ZONE == z) %>% dplyr::select(-AFL_ZONE) %>% preprocess(response = RESPONSE, predictors = considered_columns)
  X <- X_total$main
  
  mean(X$TIME_IN_POSS_SQUAD_RATE, na.rm = TRUE)
  ...
  png(filename = paste0("winning_plots/", z, "_var_imp.png"))
  print(plotter)
  dev.off()
  ...
  for(i in 1:100){
    if(i <= i_max){
      ...
      png(filename = paste0("winning_plots/", z, "_", str_replace_all(var, "[[:punct:]]", " "), ".png"))
    print(pr[[i]])
    dev.off()
    }
  }
