...
for(n in names(df)){
  df[[n]][is.infinite(df[[n]])] <- NA
}

# target_file = "team_summary"
target_file = "team_rates"
df_rates <- readRDS(paste0("clean_data/", target_file, ".rds"))
for(n in names(df_rates)){
  df_rates[[n]][is.infinite(df_rates[[n]])] <- NA
}
...
umap_2_train_base <- umap_2_train
...
png("output/umap_teams.png", width = 1600, height = 1000)
plot(umap_2_train_plot)
dev.off()
...
cluster = umap_2_train %>% as.matrix() %>% hdbscan(minPts = 40, gen_hdbscan_tree = T, gen_simplified_tree = T)
...
## Overlay on 2D UMAP output - With Assigned Cluster Names
```{r fig.width=10, fig.height=6}
umap_2_cluster <- umap_2_train %>% 
  mutate(Cluster = cluster$cluster)
...
# Overall Influence
```{r fig.width=10, fig.height=6}
influencing_variables <- imp$Variable[1:10]

umap_2_train <- umap_2_train_base
...
threshold = 0.1
# For each influencing_variable, assign the colour column if the values are in the top of the range
for(var in influencing_variables){
  # Can't consider factor variables
  if(!("factor" %in% class(dt[[var]]))){
    # Find the appropriate quantile
    range_high = dt[[var]] %>% quantile(1-threshold)
    range_low = dt[[var]] %>% quantile(threshold)
    
    # Any unassigned colour whose variable is above this threshold is assigned the variable name
    dt$Colour[is.na(dt$Colour) & dt[[var]] > range_high] <- paste0(var, " (High)")
    # dt$Colour[is.na(dt$Colour) & dt[[var]] < range_low] <- paste0(var, " (Low)")
  }
}

dt$Colour[is.na(dt$Colour)] <- "None"

umap_2_train_plot <- dt %>% 
  ggplot(
    aes(
      x = umap_1,
      y = umap_2,
...
this_zone <- df_rates %>% 
  filter(
    ZONE_LOGICAL_AFL %in% c("AM", "DM")
  )

# influencing_variables <- c(
...
# Get the dataset
X_total <- this_zone %>% preprocess(response = RESPONSE, predictors = c(considered_columns), drop_at_na_level = 0.99)
X <- X_total$main
...
# Get the dataset
X_total <- this_zone %>% preprocess(response = RESPONSE, predictors = c(considered_columns))
X <- X_total$main
