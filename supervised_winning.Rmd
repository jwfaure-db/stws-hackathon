...
# Want a new variable to state whether a team won or lost and drop SQUAD_MARGIN
df <- df %>% dplyr::mutate(RESULT = ifelse(SQUAD_MARGIN == 0, "draw", ifelse(SQUAD_MARGIN > 0, "won", "lost")), .keep = "unused")

# new variable stating whether team of focus is home or away, and drop similar columns
df <- df %>% dplyr::mutate(HOME_AWAY = ifelse(SQUAD_NAME == HOME_SQUAD, "home", "away")) %>% dplyr::select(-c("HOME_SQUAD_ID", "HOME_SQUAD", 
                                                                                                              "AWAY_SQUAD_ID", "AWAY_SQUAD", "VENUE_LOCATION",
...
get_test_results(xgb, test) %>% metrics(truth = Observed, estimate = Predicted)
